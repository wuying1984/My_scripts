...
#my %name2se;
my %name2sp;
my @anno_tab = glob "*_all_annotation.tab";
my @gff_file = glob "*.final.gff";
print join ";", @anno_tab, "\n";
print join ";", @gff_file, "\n";
foreach my $file (@anno_tab) {
	...
	print scalar @lines, "\n";
        ...
        foreach my $line (@lines){
                ...
                my $name = $eles[0];
                my $partial = "N";
                $partial = "Y" if $name2partical{$name};
                my $sp = $eles[6];
                my $spnotm = $eles[6];
                my $hitother = $eles[13];
                if ($sp eq "Y"){
                ...
                $name2complete{$name}="complete" if $partial eq "N";
                $name2complete{$name}=$name2partical{$name} if $partial eq "Y";
        }
}
...
foreach my $file2 (@anno_tab){
        ...
        foreach my $line (@lines){
                ...
                my $desc=$eles[8];
                $desc = "hypothetical protein" if $desc =~ /\-NA\-/;
                $desc =~ s/\(.*\)// if $desc =~ /\(.*\)/;
                $desc =~ s/ partial//g if $desc =~ / partial/;
                $desc =~ s/partial//g if $desc =~ /partial/;
                $desc = "hypothetical protein" if $desc eq "domain";
		...
		$desc =~ s/\, $// if $desc =~ /\, $/;
                $desc = $desc . " partial sequence" if $name2partical{$name};
                $name2desc{$name}= $desc;
...
foreach my $file3 (@gff_file){
		...
		if (!$gene2remove{$name}){
		  #               print $id, "\n"; sleep 1;
		  $contig2gene{$isolate}{$contig}{$name} ++ if $name2complete{$name};
		  #               print $name, "\n";
		  #               sleep 1;
...
foreach my $contig (sort { $a cmp $b} keys %{$contig2gene{$isolate}}){
        ...
        $contig_new =~ s/Gc_UCSC1/GcC1/ if $contig =~ /UCSC1/;
        ...
        foreach my $name (sort {$a cmp $b} keys %{$contig2gene{$isolate}{$contig}}){
                ...
                my $new_id = $id; $new_id =~ s/$isolate/$prefix/;
		...
		print W "\t\t\tproduct\t$desc\n";
                print W "\t\t\tprotein_id\tgnl\|XiaoUMD\|$new_id\n";
                print W "\t\t\ttranscript_id\tgnl\|XiaoUMD\|$new_id","T\n";
